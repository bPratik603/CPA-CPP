...
class Date{
    ...
    public: 
        ...
        bool operator<=(const Date& other){
            bool left_smaller_equal; 

            if(year != other.year)
                left_smaller_equal = (year <= other.year);  
            else if(month != other.month)
                left_smaller_equal = (month <= other.month);
            else 
                left_smaller_equal = (day <= other.day);  

            return left_smaller_equal; 
        }
