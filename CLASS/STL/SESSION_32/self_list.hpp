    ...
    namespace dsa{
        ...
        class list{
            private:
               ...
               static void generic_insert(node<T>* beg,node<T>* mid,
               node<T>* end){
                  mid->next = end;
                  mid->prev = beg;
                  beg->next = mid;
               ...
               static void genric_delete(node<T>* delete_node)
               {
           ...
           status_t insert_after(
            const T& e_data,const T& new_data){
                node<T>* e_node=0;
                e_node = search_node(e_data);
                if(e_node == 0)
                   return LIST_DATA_NOT_FOUND;

              generic_insert(e_node->next,new node<T>(new_data),e_node);
              return SUCCESS;
           }
           ...
           status_t insert_before(const T& e_data,const T& new_data)
           {
               ...
               genric_insert(e_node->prev,new node<T>(new_data),e_node);
               return SUCCESS;
           }

           statsu_t get_start(T* p_start_data){
           ...
           const list<T>& list_object){
             os<<"[START]->";
             for(node<T>* run = list_object.head_node->next;
                run!=list_object.head_node,run=run->next)
                {
                    os<<"["<<run->data<<"]<->";
                }
              os<<"[END]"<<std::endl;
          ...
          iterator end(){
            return iterator(head_node->prev);
          }
